...
class TestSrssStoreBasic:
    def test_that_schema_version_is_set(self, fixture_temp_db):
        ...
        assert db.rowExists(db.cursor(), 'SELECT * FROM shinerainsoftsevenutil_store_properties WHERE schema_version==1')

    def test_that_wrong_schema_version_is_not_set(self, fixture_temp_db):
        db, _ = fixture_temp_db
        assert not db.rowExists(db.cursor(), 'SELECT * FROM shinerainsoftsevenutil_store_properties WHERE schema_version==2')
    ...
    def test_opening_with_prev_schema_version(self, fixture_temp_db):
        db, dbpath = fixture_temp_db
        db.cursor().execute('UPDATE shinerainsoftsevenutil_store_properties SET schema_version=0 WHERE 1')
        db.close()
        with pytest.raises(SrssStoreException) as exc:
            db.connectOrCreate(dbpath)
    ...
    def test_opening_with_future_schema_version(self, fixture_temp_db):
        db, dbpath = fixture_temp_db
        db.cursor().execute('UPDATE shinerainsoftsevenutil_store_properties SET schema_version=2 WHERE 1')
        db.close()
        with pytest.raises(SrssStoreException) as exc:
            db.connectOrCreate(dbpath)
...
@pytest.fixture()
def fixture_temp_db():
    basedir = join(tempfile.gettempdir(), u'shinerainsoftsevenutil_test', u'empty')
