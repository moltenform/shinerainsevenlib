...
class SrssStoreBasic:
    ...
    def stampSchemaVersion(self, cursor):
        ...
        cursor.execute('CREATE TABLE shinerainsevenlib_store_properties(schema_version INT)')
        cursor.execute(
            'INSERT INTO shinerainsevenlib_store_properties(schema_version) VALUES(?)',
    ...
    def verifySchemaVersion(self):
        ...
        try:
            ...
            for version in cursor.execute(
                'SELECT schema_version from shinerainsevenlib_store_properties'
            ):
                got = int(version[0])
